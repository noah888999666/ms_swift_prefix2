...
from ..constant import LLMTemplateType, MLLMTemplateType, DEFAULT_SYSTEM
from ..register import register_template
from ..template_inputs import StdTemplateInputs
from ..template_meta import TemplateMeta
from ..utils import Context, Word, findall, Prompt
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel):
    """Qwen2.5-VL model for conditional generation tasks."""
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Forward pass for the Qwen2.5-VL model.
        
        Process:
        1. Process text and image inputs through base model
        2. Generate logits from hidden states
...
class Qwen2_5VLWithPrefixTemplate(Qwen2_5VLTemplate):
    """Template for Qwen2.5-VL model with text prefix support."""
    version = 'v2_5'
    norm_bbox = 'none'
    support_system = True
    ...
    def process_inputs(
        ...
        **kwargs,
        ...
        Args:
            ...
            messages: Original messages containing instruction from dataset
            **kwargs: Additional arguments
            
        Returns:
            Dict containing processed inputs
        """
        inputs = super().process_inputs(input_ids, attention_mask, tokenizer, messages, **kwargs)

        # Extract instruction from messages if available
        if messages:
            instruction = None
            ...
            if instruction:
                # Process instruction as text prefix
                ...
                inputs.update({
                    'text_prefix_ids': prefix_tokens['input_ids'].to(input_ids.device),
                    'text_prefix_attention_mask': prefix_tokens['attention_mask'].to(input_ids.device),
                })

        return inputs

# Register the template with system message support
register_template(
    TemplateMeta(
        template_type=MLLMTemplateType.qwen2_5_vl_with_prefix,
        prefix=['<s>'],  # Start with BOS token
        prompt=['<|im_start|>user\n{{QUERY}}<|im_end|>\n<|im_start|>assistant\n'],
        chat_sep=['<|im_end|>\n'],
        suffix=['<|im_end|>'],
        system_prefix=['<s><|im_start|>system\n{{SYSTEM}}<|im_end|>\n'],  # Support system messages
        template_cls=Qwen2_5VLWithPrefixTemplate,
        stop_words=['<|im_end|>'],
    ),
    exist_ok=True  # Allow overriding existing registration
)
