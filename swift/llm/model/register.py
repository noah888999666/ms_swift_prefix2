...
def get_model_tokenizer_multimodal(model_dir: str, model_info=None, model_kwargs=None, load_model=True, **kwargs):
    from transformers import AutoProcessor
    import os
    import sys
    from swift.llm.template.template.modeling_qwen2_5_vl_with_prefix import (
        Qwen2_5_VLWithPrefixConfig,
        Qwen2_5_VLWithPrefixForConditionalGeneration,
    )
    
    # Add our custom model directory to Python path
    custom_model_dir = os.path.dirname(os.path.abspath(__file__))
    if custom_model_dir not in sys.path:
        sys.path.append(custom_model_dir)
    
    # Get model_info from either positional args or kwargs
    if model_info is None:
        model_info = kwargs.get('model_info')
    
    # Create our custom config
    if model_info is not None and model_info.model_type == 'qwen2_5_vl_with_prefix':
        # Create symbolic link to our custom model file
        custom_model_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 
                                       'template/template/modeling_qwen2_5_vl_with_prefix.py')
        target_model_path = os.path.join(model_dir, 'modeling_qwen2_5_vl_with_prefix.py')
        
        if not os.path.exists(target_model_path):
            os.symlink(custom_model_path, target_model_path)
        
        config = Qwen2_5_VLWithPrefixConfig.from_pretrained(model_dir, trust_remote_code=True)
        kwargs['model_config'] = config
        kwargs['automodel_class'] = Qwen2_5_VLWithPrefixForConditionalGeneration
...
def get_model_tokenizer(
    ...
    # Remove model_info from kwargs since it's passed as a positional arg
...
# Register our custom prefix model
register_model(
    ModelMeta(
        model_type="qwen2_5_vl_with_prefix",
        model_groups=[
            ModelGroup(
                models=[
                    Model(
                        ms_model_id="Qwen/Qwen2.5-VL-3B-Instruct",
                        hf_model_id="Qwen/Qwen2.5-VL-3B-Instruct",
                    )
                ]
            )
        ],
        template="qwen2_5_vl_with_prefix",  # Matches our template type
        get_function=get_model_tokenizer_multimodal,  # Use multimodal loader
        is_multimodal=True,  # This is a multimodal model
        architectures=["Qwen2_5_VLWithPrefixForConditionalGeneration"],  # Our custom architecture
        model_arch="qwen2_5_vl_with_prefix",  # Specify the model architecture
    )
)
